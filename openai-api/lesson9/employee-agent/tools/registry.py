...
# Dispatcher
# ==========================================================
def execute_tool(tool_name, arguments, event_callback=None ):
    function = FUNCTIONS.get(tool_name)

    if function is None:
        raise ValueError(f"Tool not authorized: {tool_name}")

    if event_callback:
        event_callback("TOOL_STARTED", f"Calling {tool_name}")
    ...
    try:
        ...
        logger.info("Tool=%s duration=%.3fs", tool_name,elapsed)

        if event_callback:
            event_callback("TOOL_COMPLETED", ( f"{tool_name} " f"completed in " f"{elapsed:.3f}s"))
        return result
    except Exception as exc:
        elapsed = (time.perf_counter() - start_time)
        logger.exception("Tool=%s failed after %.3fs", tool_name, elapsed)

        if event_callback:
            event_callback("TOOL_FAILED", (f"{tool_name} " f"failed after " f"{elapsed:.3f}s"))
        raise